...
var _ = g.Describe("[Serial] API data in etcd", func() {
	...
	_ = g.It("should be stored at the correct location and version for all resources", func() {
		...
		etcdConfigMap, err := coreV1.ConfigMaps("openshift-config").Get(context.TODO(), "etcd-ca-bundle", metav1.GetOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		etcdSecret, err := coreV1.Secrets("openshift-config").Get(context.TODO(), "etcd-client", metav1.GetOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
